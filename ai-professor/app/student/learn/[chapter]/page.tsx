...
export default function LearnPage({
  ...
  function mark(understood: boolean) {
    ...
    if (understood && !isLast) setIdx((i) => i + 1);
  }
  ...
  return (
    <div className="flex h-screen flex-col bg-white">
      ...
      {/* 본문: 왼쪽 슬라이드 / 오른쪽 실시간 설명·챗 */}
      <div className="grid flex-1 grid-cols-1 overflow-hidden md:grid-cols-2">
        <div className="hidden overflow-hidden border-r border-zinc-200 bg-zinc-100 md:block">
          {chapter.pdfUrl ? (
            // 업로드된 실제 슬라이드를 그대로 표시 (브라우저 내장 PDF 뷰어).
            // slide 번호 = PDF 페이지. key로 페이지 전환 시 해당 페이지로 이동시킨다.
            <iframe
              key={note.slide}
      ...
      <div className="border-t border-zinc-200 bg-white px-5 py-3.5">
        ...
        {isLast && (
          <p className="mt-3.5 text-center text-sm text-zinc-500">
