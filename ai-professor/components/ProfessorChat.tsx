...
// 스트리밍 응답 본문을 델타 단위로 읽어 콜백. 타이핑 효과의 핵심.
...
export default function ProfessorChat({ chapter, slide, personaId }: Props) {
  ...
  // 슬라이드 또는 페르소나가 바뀌면 설명을 새로 스트리밍한다.
  useEffect(() => {
    let cancelled = false;
    setExplanation("");
    setQas([]);
    ...
    (async () => {
      try {
        const res = await fetch("/api/explain", {
          ...
          body: JSON.stringify({ chapter, slide, personaId }),
        });
  ...
  async function ask(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/qna", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ chapter, slide, personaId, question: q }),
      });
      await readStream(res, (t) => {
        setQas((prev) => {
          const next = [...prev];
